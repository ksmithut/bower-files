  ...
  describe('Level 1', function () {
    it('should use the override', function () {
      ...
      var pathToJquery =
        './' +
        FIXTURES +
        '/override/bower_components/jquery/dist/jquery.min.js'
    ...
    it('should use the main file when self option is set', function () {
      ...
      expect(files.js[2]).to.be.equal(
        path.join(__dirname, FIXTURES, 'self-main/lib/fastclick.js')
      )
    })
    ...
    it('should join extensions into a single property', function () {
      cd('join')
      var files = getModule({
        join: { fonts: ['eot', 'svg', 'ttf', 'woff', 'otf'] }
      })
      expect(files).to.be.instanceOf(Object)
    ...
    it('should follow symbolic links', function () {
      ...
      var dest = path.join(
        __dirname,
        FIXTURES,
        'symlinks/bower_components/angular'
      )
      var expectedJS = path.join(
        __dirname,
        FIXTURES,
        'symlinks/bower_components/angular/angular.js'
      )
      try {
