...
module.exports = function bowerFiles(options) {
  ...
  // If the self option is set, get the main file from the given bower.json
  if (options.self && bower.main) {
    files = files.concat(normalizeMain(bower.main, options.cwd));
  }
...
function getDependencies(dependencies, options) {
  ...
  Object.keys(dependencies || {}).map(function (dependency) {
    ...
    var deps = (override && override.dependencies) || component.dependencies;
    // Put the child dependencies onto the files array
    files = files.concat(getDependencies(deps, options));
    ...
    files = files.concat(normalizeMain(main, componentPath));
  });
  return uniques(files);
}

function normalizeMain(main, dir) {
  var files = [];
  arrayify(main).map(function (file) {
...
// --------
//
// Makes a variable an array if it isn't an array already
