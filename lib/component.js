...
function Component(options) {
  ...
  this.name = this.json.name || path.basename(options.dir);
  var overrides = assign({}, this.json.overrides, options.overrides);
  options.overrides = overrides;
  assign(this.json, overrides[this.name]);

  this.files = mainFiles(options.dir, this.json.main);
  ...
  // TODO deprecate
  this.dir = options.dependencyDir;
  this.path = options.dir;
  this.name = options.isRoot ? 'self' : this.name;
}
...
Component.prototype = {
  getDependencies: function (options) {
    ...
    var components = [];
    if (options.dev) { components = components.concat(this.devDependencies || []); }
    if (options.main) { components = components.concat(this.dependencies || []); }
