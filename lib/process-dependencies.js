...
function processDependencies(dependencies, baseDir, overrides) {
  ...
  dependencies.some(function (dependency) {
    ...
    // If the path is a symlink (i.e. created by `bower link ...`), look for
    // 'bower.json' instead of '.bower.json'.
    if (exists(componentPath) && isSymlink(componentPath)) {
    ...
    // Try to get the component's JSON manifest. If it doesn't exist, it likely
